...
public class ExprTokenizationTests {
	...
	public void testNumberIsParsedAsANumber() {
		context.checking(new Expectations() {{
			oneOf(builder).term(with(ExprMatcher.number(42).location(fred, 1, 0, 2))); inSequence(order);
			allowing(errors).hasErrors(); will(returnValue(true)); // there actually aren't, but it stops the reduction error firing ...
			oneOf(builder).done(); inSequence(order);
		}});
	...
	public void testStringInSingleQuotesIsParsedAsALiteral() {
		context.checking(new Expectations() {{
			oneOf(builder).term(with(ExprMatcher.string("hello").location(fred, 1, 0, 7))); inSequence(order);
			allowing(errors).hasErrors(); will(returnValue(true)); // there actually aren't, but it stops the reduction error firing ...
			oneOf(builder).done(); inSequence(order);
		}});
	...
	public void testPlusIsParsedAsASymbol() {
		context.checking(new Expectations() {{
			oneOf(builder).term(with(ExprMatcher.operator("+").location(fred, 1, 0, 1))); inSequence(order);
			allowing(errors).hasErrors(); will(returnValue(true)); // there actually aren't, but it stops the reduction error firing ...
			oneOf(builder).done(); inSequence(order);
		}});
		assertTrue(parser.tryParsing(new Tokenizable("+")) instanceof IgnoreNestedParser);
	...
	public void testORBIsParsedAsAPunc() {
		context.checking(new Expectations() {{
			oneOf(builder).term(with(ExprMatcher.punc("(").location(fred, 1, 0, 1))); inSequence(order);
			allowing(errors).hasErrors(); will(returnValue(true)); // there actually aren't, but it stops the reduction error firing ...
			oneOf(builder).done(); inSequence(order);
		}});
		assertTrue(parser.tryParsing(new Tokenizable("(")) instanceof IgnoreNestedParser);
	...
	public void testDotIsParsedAsASymbol() {
		context.checking(new Expectations() {{
			oneOf(builder).term(with(ExprMatcher.dot().location(fred, 1, 0, 1))); inSequence(order);
			allowing(errors).hasErrors(); will(returnValue(true)); // there actually aren't, but it stops the reduction error firing ...
			oneOf(builder).done(); inSequence(order);
		}});
		assertTrue(parser.tryParsing(new Tokenizable(".")) instanceof IgnoreNestedParser);
	...
	public void testVarIsParsedAsAnUnresolvedVar() {
		context.checking(new Expectations() {{
			oneOf(builder).term(with(ExprMatcher.unresolved("x").location(fred, 1, 0, 1))); inSequence(order);
			allowing(errors).hasErrors(); will(returnValue(true)); // there actually aren't, but it stops the reduction error firing ...
			oneOf(builder).done(); inSequence(order);
		}});
	...
	public void testNilBecomesAConstructorAsAValue() {
		context.checking(new Expectations() {{
			oneOf(builder).term(with(ExprMatcher.typeref("Nil").location(fred, 1, 0, 3))); inSequence(order);
			allowing(errors).hasErrors(); will(returnValue(true)); // there actually aren't, but it stops the reduction error firing ...
			oneOf(builder).done(); inSequence(order);
		}});
	...
	public void testASequenceOfItems() {
		context.checking(new Expectations() {{
			oneOf(builder).term(with(ExprMatcher.number(42).location(fred, 1, 0, 2))); inSequence(order);
			oneOf(builder).term(with(ExprMatcher.operator("+").location(fred, 1, 3, 4))); inSequence(order);
			oneOf(builder).term(with(ExprMatcher.unresolved("f").location(fred, 1, 5, 6))); inSequence(order);
			oneOf(builder).term(with(ExprMatcher.string("hello").location(fred, 1, 7, 14))); inSequence(order);
			allowing(errors).hasErrors(); will(returnValue(true)); // there actually aren't, but it stops the reduction error firing ...
			oneOf(builder).done(); inSequence(order);
		}});
	...
	public void testASequenceOfItemsWithoutSpaces() {
		context.checking(new Expectations() {{
			oneOf(builder).term(with(ExprMatcher.number(42).location(fred, 1, 0, 2))); inSequence(order);
			oneOf(builder).term(with(ExprMatcher.operator("+").location(fred, 1, 2, 3))); inSequence(order);
			oneOf(builder).term(with(ExprMatcher.unresolved("f").location(fred, 1, 3, 4))); inSequence(order);
			oneOf(builder).term(with(ExprMatcher.string("hello").location(fred, 1, 4, 11))); inSequence(order);
			allowing(errors).hasErrors(); will(returnValue(true)); // there actually aren't, but it stops the reduction error firing ...
			oneOf(builder).done(); inSequence(order);
		}});
		assertTrue(parser.tryParsing(new Tokenizable("42+f'hello'")) instanceof IgnoreNestedParser);
	...
	public void anErrorInStringParsingIsAtACrediblePlace() {
		context.checking(new Expectations() {{
			...
			oneOf(errors).message(new InputPosition(fred, 1, 15, null, ""), "unterminated string");
			oneOf(builder).done();
		}});
