...
public class DoUTInputGeneratorJS extends LeafAdapter implements ResultAware {
	...
	public DoUTInputGeneratorJS(JSFunctionState state, NestedVisitor sv, JSBlockCreator block, JSExpr runner) {
		this.state = state;
		this.sv = sv;
		this.block = block;
		this.runner = runner;
	...
	public void leaveUnitTestInput(UnitTestInput e) {
		if (args.size() != 2)
			throw new RuntimeException("expected card & data");
		// TODO: this needs to be an array of [type, name] elements to traverse the tree
		block.assertable(runner, "input", args.get(0), makeSelector(block, e.targetZone), args.get(1));
		sv.result(null);
	}
