...
public class SwitchVars {
	...
	public AVar get(List<IExpr> currentBlock, Slot slot) {
		...
		if (!(e instanceof AVar)) {
			...
			if (state.ocret() != null) {
				...
				currentBlock.add(meth.ifBoolean(meth.instanceOf(var, J.RESPONSE_WITH_MESSAGES), meth.block(extract, reassign), null));
			}
