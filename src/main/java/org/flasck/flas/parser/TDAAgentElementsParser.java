...
public class TDAAgentElementsParser extends BlockLocationTracker implements TDAParsing, FunctionNameProvider, HandlerNameProvider {
	...
	public TDAParsing tryParsing(Tokenizable toks) {
		...
		case "provides": {
			...
			final Provides contractService = new Provides(kw.location, tn.location, (NamedType)consumer, ctr, csn, varloc, varname);
			consumer.addProvidedService(contractService);
			if (varname != null) {
				topLevel.newProvidesServiceWithName(errors, contractService);
			}
			return new TDAParsingWithAction(
		...
		case "method": {
			...
			TDAMethodParser mp = new TDAMethodParser(errors, this.namer, smConsumer, topLevel, holder, this, null, "method-intro", true);
			return new TDAParsingWithAction(
				mp.parseMethod(kw, namer, toks),
				null // mp.reduction(kw.location, "agent-method-definition")
			);
		}
		default:
